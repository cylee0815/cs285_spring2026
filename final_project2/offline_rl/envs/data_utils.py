...
def _build_flat_features(
    ...
    macro_arr: Optional[np.ndarray] = None,
    sentiment_arr: Optional[np.ndarray] = None,
) -> np.ndarray:
    """Flatten (T, N_assets, N_feat) → (T, F) and optionally concat macro/sentiment."""
    T = features_3d.shape[0]
    parts = [features_3d.reshape(T, -1)]
    if macro_arr is not None:
        parts.append(macro_arr)
    if sentiment_arr is not None:
        parts.append(sentiment_arr)
...
def make_train_test_envs(
    ...
    **env_kwargs,
    ...
    Uses ``env.portfolio_env.PortfolioEnv`` (the project's canonical env).
    ...
    def _make_env(s, e):
        return PortfolioEnv(
            features=flat_features[s:e],
            forward_returns=forward_returns[s:e],
