...
disabled, and the exogeneity unit tests
(Sec.~\ref{sec:method:grpo:exogeneity}) guard the constraint.
...
$10^{-6}$ clamp). (As a side effect, the $+1$ shift means the head
cannot exactly represent the uniform distribution $\alpha{=}1$; in
practice the policy approaches but does not reach the simplex
centroid.)
...
A subtle but important detail: when all $G$ candidates score identical
...
advantages are biased. The exogeneity unit tests
(Sec.~\ref{sec:method:grpo:exogeneity}) are not stylistic --- they are
an algorithmic precondition. We surface this
...
- \tfrac{1}{2}.
\end{equation}
Intuitively, this measures how far the rolling distribution of online
encoder outputs has drifted from the offline reference: small values
when the deployment regime resembles the offline buffer, larger values
...
\paragraph{Online-loop entrypoint.} All Phase~2C adaptive-vs-naive
numbers were collected after a fix to the online entrypoint that
ensures the schedule $\beta_t$ is actually forwarded to the critic
update; the fix, and three other pre-existing bugs found during Phase~2
prep, are documented in Appendix~\ref{app:implementation}.
