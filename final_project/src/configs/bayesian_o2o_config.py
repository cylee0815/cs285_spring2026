...
def get_config():
    ...
    config.regime_kl_scale = 0.5   # lambda in sigmoid(lambda * KL + gamma * Var)
    # Bayesian regime encoder
    config.bayesian = True
    config.kl_beta = 0.1           # weight of KL(posterior || prior) regularization
    config.var_scale = 1.0         # gamma: weight of Var[h_t] in CQL penalty
    return config
