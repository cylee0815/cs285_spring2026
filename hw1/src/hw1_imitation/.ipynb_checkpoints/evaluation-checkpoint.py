...
class Logger:
    ...
    def __init__(self, path: Path):
        ...
        ### My revision to make logging faster
        # 1. Define columns once. 
        # This prevents the need to rewrite the file when new metrics appear.
        self.columns = ["step", "epoch", "train/loss", "eval/mean_reward"]
        
        # 2. Write header immediately.
    ...
    ### My revision: rewrite log() to log both loss and reward in the same csv for plotting
    def log(self, row: dict[str, Any], step: int) -> None:
        row["step"] = step
        
        # 3. Extract values for the known columns. 
        # If a column (like eval/mean_reward) is missing in this step, 
        # .get() returns "" (empty string), which pandas handles correctly as NaN.
        values = [str(row.get(k, "")) for k in self.columns]
        
        # 4. Append mode ("a") is fast. 
        # We perform one I/O operation per log instead of rewriting the whole file.
        with self.csv_path.open("a") as f:
            f.write(",".join(values) + "\n")
